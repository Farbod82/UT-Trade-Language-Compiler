...
mainDeclaration returns [ MainDeclaration mainDecRet] : { $mainDecRet = new MainDeclaration(); } VOID MAIN LPAREN RPAREN (LBRACE (statement {$mainDecRet.setLine($MAIN.line);$mainDecRet.addStatement($statement.statementRet);})* RBRACE | statement{ $mainDecRet.addStatement($statement.statementRet);})
      {
      for(Statement stmt : $mainDecRet.getBody()){
           if(stmt instanceof VarDeclaration){
           VarDeclaration vardec = (VarDeclaration)stmt;
            if(vardec.getType() instanceof TradeType){
                $mainDecRet.addTrade(vardec);
            }
           }
      }
      }
...
initDeclaration returns [OnInitDeclaration initDecRet] : {$initDecRet = new OnInitDeclaration();}
             VOID ONINIT LPAREN TRADE identifier {$initDecRet.setLine($ONINIT.line); $initDecRet.setTradeName($identifier.identRet);} RPAREN (THROW EXCEPTION)? (LBRACE (statement{$initDecRet.addStatement($statement.statementRet);})*  RBRACE |
              statement {$initDecRet.addStatement($statement.statementRet);});

startDeclaration returns [OnStartDeclaration startDecRet]: {$startDecRet = new OnStartDeclaration(null);} VOID ONSTART LPAREN TRADE{$startDecRet.setLine($VOID.line);}
 identifier {$startDecRet.setTradeName($identifier.identRet);} RPAREN (THROW EXCEPTION)? (LBRACE (statement {$startDecRet.addStatement($statement.statementRet);})* RBRACE | statement {$startDecRet.addStatement($statement.statementRet);});

assignStatement returns [AssignStmt assignStmtRet] : {$assignStmtRet = new AssignStmt(null,null);}(i1=identifier {$assignStmtRet.setLValue($i1.identRet);}|( i2=identifier LBRACK ex1=expression RBRACK{ArrayIdentifier arrayRet = new ArrayIdentifier($i2.identRet.getName(),$ex1.expressionRet);arrayRet.setLine($i2.identRet.getLine());$assignStmtRet.setLValue(arrayRet);})) assign ex2 = expression {$assignStmtRet.setRValue($ex2.expressionRet);} SEMICOLON{$assignStmtRet.setLine($SEMICOLON.line);};

ifStatement returns [IfElseStmt ifStmtRet] : {$ifStmtRet = new IfElseStmt(null);} IF {$ifStmtRet.setLine($IF.line);} LPAREN expression {$ifStmtRet.setCondition($expression.expressionRet);} RPAREN (LBRACE (statement {$ifStmtRet.addThenStatement($statement.statementRet);})* RBRACE | statement{$ifStmtRet.addThenStatement($statement.statementRet);}) (ELSE (LBRACE (statement{$ifStmtRet.addElseStatement($statement.statementRet);})* RBRACE | statement{$ifStmtRet.addElseStatement($statement.statementRet);}))?;

whileStatement returns [WhileStmt whileStmtRet] : {$whileStmtRet = new WhileStmt(null);} WHILE {$whileStmtRet.setLine($WHILE.line);} LPAREN expression {$whileStmtRet.setCondition($expression.expressionRet);} RPAREN (LBRACE (statement {$whileStmtRet.addBody($statement.statementRet);})* RBRACE | statement{$whileStmtRet.addBody($statement.statementRet);});

forStatement returns [ForStmt forStmtRet] : {$forStmtRet = new ForStmt();} FOR {$forStmtRet.setLine($FOR.line);} LPAREN statement {$forStmtRet.addInit($statement.statementRet);} expression {$forStmtRet.setCondition($expression.expressionRet);} SEMICOLON (expression{$forStmtRet.addUpdate($expression.expressionRet);})? RPAREN (LBRACE (statement {$forStmtRet.addBody($statement.statementRet);})* RBRACE | statement {$forStmtRet.addBody($statement.statementRet);});

tryCatchStatement returns [TryCatchStmt tryCatchStmtRet] : {$tryCatchStmtRet = new TryCatchStmt();} TRY {$tryCatchStmtRet.setLine($TRY.line);} (LBRACE (statement{$tryCatchStmtRet.addThenStatement($statement.statementRet);})* RBRACE | statement{$tryCatchStmtRet.addThenStatement($statement.statementRet);}) (CATCH EXCEPTION ID (LBRACE (statement{$tryCatchStmtRet.addElseStatement($statement.statementRet);})* RBRACE | statement{$tryCatchStmtRet.addElseStatement($statement.statementRet);}))?;

continueBreakStatement returns [ContinueBreakStmt contBreakRet]: (BREAK {$contBreakRet = new ContinueBreakStmt($BREAK.text);$contBreakRet.setLine($BREAK.line);} | CONTINUE {$contBreakRet = new ContinueBreakStmt($CONTINUE.text); $contBreakRet.setLine($CONTINUE.line);}) SEMICOLON;

returnStatement returns [ReturnStmt retStmtRet] : RETURN expression {$retStmtRet = new ReturnStmt($expression.expressionRet);$retStmtRet.setLine($RETURN.line);}SEMICOLON;

throwStatement returns [ThrowStmt throwStmtRet]: THROW expression{$throwStmtRet = new ThrowStmt($expression.expressionRet);$throwStmtRet.setLine($THROW.line);} SEMICOLON;

functionCall returns [FunctionCall funcCallRet]: {$funcCallRet = new FunctionCall(null);} (espetialFunction {$funcCallRet.setFunctionName($espetialFunction.name);}| complexValue{$funcCallRet.setFunctionName($complexValue.compTypeRet);} | identifier{$funcCallRet.setFunctionName($identifier.identRet);}) LPAREN (expression{$funcCallRet.addArg($expression.expressionRet);} (COMMA expression{$funcCallRet.addArg($expression.expressionRet);})*)? RPAREN{$funcCallRet.setLine($LPAREN.line);};

methodCall returns [MethodCall methodCallRet] : {$methodCallRet = new MethodCall(null,null);} (identifier {$methodCallRet.setInstance($identifier.identRet);}
...
primitiveType returns [Type primTypeRet] :
        ...
        | DOUBLE {$primTypeRet = new DoubleType();}
